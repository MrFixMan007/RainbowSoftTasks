...
func main() {
	...
	for i, v := range srcs {
		wg.Add(1)
		go RequestGoroutine(dst, v, i, wg)
	}
	wg.Wait()
...
func RequestGets(v string) ([]byte, error) {
	resp, err := http.Get(v)
	if err != nil {
		return nil, fmt.Errorf("ошибка с запросом: %s", err)
	}
...
// RequestGoroutine создаём горутину, выполняющую запрос
func RequestGoroutine(dst string, v string, i int, wg *sync.WaitGroup) {
	defer wg.Done()
	defer fmt.Printf("Процесс %d завершился\n", i)
	fmt.Printf("Запустился процесс %d\n", i)
	body, err := RequestGets(v)
	if err != nil {
		fmt.Printf("в процессе %d %s\n", i, err)
		return
	}
